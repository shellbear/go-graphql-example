...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withTodos; query != nil {
		...
		for _, n := range neighbors {
			fk := n.todo_user
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "todo_user" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "todo_user" returned %v for node %v`, *fk, n.ID)
			}
